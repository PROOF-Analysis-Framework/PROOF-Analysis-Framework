...
void PAFBasicSample::Check() {
    ...
    for (Int_t i = 0; i < this->sample->GetListOfElements()->GetSize(); i++) {
        TDSetElement *firstElement = (TDSetElement *) this->sample->GetListOfElements()->At(i);

        if (firstElement == NULL) {
            PAF_FATAL("Project", "There is no ROOT file specified.");
        }

        std::cout << "Checking file " << firstElement->GetName() << std::endl;
        /**
