...
void PAFToolCreateSelector::ExecuteTool(TList* params)
{
	...
	repositoryPath.Append(DIRECTORY).Append(selectorName);
	...
	TString fileNameTargetHeader = repositoryPath.Copy().Append(DIRECTORY).Append(selectorName).Append(HEADER_EXTENSION);
	...
	TString fileNameTargetSource = repositoryPath.Copy().Append(DIRECTORY).Append(selectorName).Append(SOURCE_EXTENSION);
